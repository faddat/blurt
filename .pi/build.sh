...
# docker buildx build --tag blurt --file .pi/Dockerfile --platform linux/arm64 --cache-from blurt:cache --cache-to blurt:cache --load --progress tty .
...
# Make the image file
fallocate -l 4G "images/blurt.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/blurt.img
